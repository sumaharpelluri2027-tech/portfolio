...
const aliases: Record<string, string> = { "case-files": "experience", thinking: "process", top: "home", mission: "home" };

function Mission({ navigate }: { navigate: (id: string) => void }) {
  const [connected, setConnected] = useState(true);
  return <section className="os-mission" aria-label="Mission control">
    <div className="os-intro"><p className="os-eyebrow"><span className="os-dot" /> SYSTEMS BUILDER · GURUGRAM, INDIA</p><span className="os-edition">PERSONAL OPERATING SYSTEM / V.03</span></div>
    ...
    <div className="os-proof-grid">
      <button className="os-proof os-proof-lime" onClick={() => navigate("impact")}><span>LESS BUSYWORK <ArrowUpRight size={18}/></span><strong>80<span>%</span></strong><p>of manual operations automated</p><div className="os-ticks" aria-hidden="true">{Array.from({length:25}, (_,i)=><i key={i} className={i<20 ? "on" : ""}/>)}</div></button>
      <button className="os-proof" onClick={() => navigate("impact")}><span>TRUST THE INPUTS <ArrowUpRight size={18}/></span><strong>95<span>%</span></strong><p>data accuracy · up from 50%</p><div className="os-mini-bars" aria-hidden="true"><i style={{width:"50%"}}/><i style={{width:"95%"}}/></div></button>
      <button className="os-proof os-revenue" onClick={() => navigate("lab")}><span>FROM ZERO TO MARKET <ArrowUpRight size={18}/></span><strong>₹3<span>L</span></strong><p>Decco revenue · first 30 days</p><div className="os-decco-thumb"><Image src="/assets/decco.webp" alt="Decco Store physical stall" fill sizes="160px"/></div></button>
      <button className="os-proof os-build-card" onClick={() => navigate("lab")}><span>ON THE WORKBENCH <Radio size={18}/></span><div className="os-wave" aria-hidden="true">{Array.from({length:20},(_,i)=><i key={i} style={{"--n":i} as React.CSSProperties}/>)}</div><h2>Meet Priya.</h2><p>A voice agent with a human handoff.</p><small>IN BUILD <ArrowRight size={15}/></small></button>
    </div>
    <div className="os-bottom-note"><span>06 builds. One instinct: make it work better.</span><a href="https://youtu.be/Ec_oS-oTlYQ" target="_blank" rel="noreferrer">Watch my campaign film <ArrowUpRight size={15}/></a></div>
  </section>;
}
...
export default function Home() {
  ...
  useEffect(() => {
    const sync = () => { const hash = location.hash.slice(1); const id = aliases[hash] || (projects.some(p=>p.id===hash) ? "lab" : hash); setSection(sections.some(s=>s.id===id) ? id : "home"); };
    sync();
  ...
  return <div className="os-app">
    ...
    <main id="workspace" className="os-workspace" ref={main} tabIndex={-1}><div className="os-screen" key={section}>{section === "home" ? <Mission navigate={navigate}/> : <ExploreSection section={section}/>}</div></main>
    ...
    {devMode&&<aside className="os-console" aria-label="System log"><button onClick={()=>setDevMode(false)} aria-label="Close system log"><X size={16}/></button><b>SUMAHAR.OS / SYSTEM LOG</b><span>version: 03 · tabbed workspace</span><span>build: {process.env.NEXT_PUBLIC_BUILD_SHA}</span><span>section: {section}</span><span>catalog: {projects.length} builds / {projects.filter(p=>p.status==="LIVE").length} live</span><span>soundtrack: Chasing the Horizon</span><span>theme: {light?"light":"dark"}</span></aside>}
  </div>;
}
